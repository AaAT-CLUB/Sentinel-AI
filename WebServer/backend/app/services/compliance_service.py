...
def sync_to_data_team(report: dict):
    ...
    try:
        ...
        if response.status_code in [200, 201]:
            ...
            print(f"❌ Sync Rejected. Status {response.status_code}: {response.text}")
        return response.status_code
    except requests.exceptions.RequestException as e:
        print(f"❌ Network Error (Is Tailscale off?): {e}")
        return None


def check_compliance_all(url: str):
    """
    Runs all 9 compliance checks: 5 OWASP controls + 4 Week 4 security checks.
    Syncs results to Team 2's database and returns the full report.
    """
    ...
    try:
        ...
        if r.status_code in [301, 302, 303, 307, 308]:
            ...
            if "evil.com" in location:
                return {"pass": False, "details": f"Open redirect detected: {location}"}
